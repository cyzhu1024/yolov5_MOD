...
class Conv(nn.Module):
    ...
    default_act = nn.SiLU()  # default activation SiLU函数会作用在每一个数字上
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolov5s.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        self.__dict__.update(locals())  # assign all variables to self 比如pt这种不是self.pt类型的变量
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        avg_out = self.f2(self.relu(self.f1(self.avg_pool(x)))) # shape(n,c,1,1)
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        avg_out = torch.mean(x, dim=1, keepdim=True) # shape(n,c,h,w) -> shape(n,1,h,w)
        max_out, _ = torch.max(x, dim=1, keepdim=True) # 只需要留下最大值
        out = torch.cat([avg_out, max_out], dim=1)
        out = self.sigmoid(self.conv(out)) # shape(n,1,h,w)
        return torch.mul(x, out) # 广播机制逐个元素相乘
